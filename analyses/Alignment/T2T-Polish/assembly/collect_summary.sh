...
if [[ $# -lt 4 ]]; then
  echo "Usage: ./collect_summary.sh species asm_version merqury_path asm.fa"
...
echo "Telomere"
seqtk telo -d 50000 $asm > $sp_ver.telo.bed
java -jar -Xmx256m $tools/T2T-Polish/assembly/bedSummarizeTelo.jar $sp_ver.telo.bed $asm.fai > ${sp_ver}.telo.summary
...
echo "Gap"
seqtk gap $asm > $sp_ver.gap.bed

rm $sp_ver.gap.tmp*
for chr_hap in $(cat $asm.fai | cut -f1 | grep "mat" | grep -v "chrM" | grep -v "random" | sort -k1,1V ) $(cat $asm.fai | cut -f1 | grep "hap1" | grep -v "chrM" | grep -v "random" | sort -k1,1V )
do
  count=`grep -c $chr_hap $sp_ver.gap.bed`
  echo -e "$count" >> $sp_ver.gap.tmp1
done
for chr_hap in $(cat $asm.fai | cut -f1 | grep "pat" | grep -v "random" | sort -k1,1V ) $(cat $asm.fai | cut -f1 | grep "hap2" | grep -v "random" | sort -k1,1V )
do
  count=`grep -c $chr_hap $sp_ver.gap.bed`
...
echo "rDNA"
rm $sp_ver.rDNA.tmp*
for chr_hap in $(cat $asm.fai | cut -f1 | grep "mat" | grep -v "chrM" | grep -v "random" | sort -k1,1V ) $(cat $asm.fa.fai | cut -f1 | grep "hap1" | grep -v "chrM" | grep -v "random" | sort -k1,1V )
do
  count=`grep -c $chr_hap $sp_ver.45S.mashmap.bed`
  echo -e "$count" >> $sp_ver.rDNA.tmp1
done
for chr_hap in $(cat $asm.fai | cut -f1 | grep "pat" | grep -v "random" | sort -k1,1V ) $(cat $asm.fa.fai | cut -f1 | grep "hap2" | grep -v "random" | sort -k1,1V )
do
  count=`grep -c $chr_hap $sp_ver.45S.mashmap.bed`
